...
public class CORESketch implements Serializable {
    ...
    transient public double[] useful_range;
...
//        zeta=zeta+(zeta-1)/2;
    ...
    public int mid_half_count_bucket() {
        ...
        System.out.println("\t\t\t\tall_bucket_tot_size="+count+"\t\t\tdata_size/2:"+rank+"\t\tprunedSize="+pruned_size);
        this.m = -1;
        return -1;
    }
    ...
    public void merge(CORESketch sketch) {
        ...
        if (card <= sketch.card) {
            for (Long2LongMap.Entry e : sketch.buckets.long2LongEntrySet()) {
                buckets.put(e.getLongKey(),
                        e.getLongValue() + buckets.getOrDefault(e.getLongKey() / diff, 0L));
            }
        } else {
            for (Long2LongMap.Entry e : buckets.long2LongEntrySet()) {
                sketch.buckets.put(e.getLongKey(),
