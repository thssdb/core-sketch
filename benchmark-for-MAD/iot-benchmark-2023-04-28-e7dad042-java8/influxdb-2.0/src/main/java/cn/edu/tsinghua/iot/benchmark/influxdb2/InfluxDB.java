...
public class InfluxDB implements IDatabase {
    ...
    try {
      ...
      InfluxDBClientOptions options =
          InfluxDBClientOptions.builder()
              ...
              .bucket(influxDbName)
              .build();
      options.getOkHttpClient().readTimeout(toDuration("2000s"));
      options.getOkHttpClient().callTimeout(toDuration("2000s"));
      options.getOkHttpClient().writeTimeout(toDuration("2000s"));
      client = InfluxDBClientFactory.create(options); // DANGER

      //      client = InfluxDBClientFactory.create(influxUrl, token.toCharArray(), org,
      // influxDbName);
  ...
  public Status aggRangeQuery(AggRangeQuery aggRangeQuery) {
    ...
    //      }
    //    } DANGER !!
    long start = aggRangeQuery.getStartTimestamp() / 1000,
        end = aggRangeQuery.getEndTimestamp() / 1000;
    String sql =
        new StringBuilder("import \"contrib/anaisdg/anomalydetection\"\n")
            .append("from(bucket: \"admin\")\n")
            .append("  |> range(start: ")
            .append(start)
            .append(", stop:")
            .append(end)
            .append(")\n")
            .append("|> anomalydetection.mad(threshold: 3.0)\n")
            .append("|> count()\n")
            .toString(); // DANGER
    Status status = executeQueryAndGetStatus(sql);
    result += status.getQueryResultPointNum();
